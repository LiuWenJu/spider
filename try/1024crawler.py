...
def crawerEach(url, urldir):
    ...
    items = soup.find('body').find('div', id='main').find(name='div', attrs={"class":"t", "style":"margin:3px auto"}
                                                          ).find('table', id='ajaxtable').find("tody").findAll(name="tr", attrs={"class":"tr3 t_one"})
    for item in items:
        ...
        if target.u == None and target.b == None and target.font == None:
            urldir[target.text] = "http://cc.bearhk.info/"+target.get('href')
    return urldir
...
def crawer():
    urldir = {}
    for i in range(39):
        url = "http://cc.bearhk.info/thread0806.php?fid"+str(i+1)
...
#以文章的标题模糊搜索
def search():
    keyword = input("please input key word: ")
